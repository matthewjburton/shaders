...
vec4 drawCircle(vec2 uv) {
    ...
    r0 = mix(mix(innerRadius, 1.0, 0.4),
             mix(innerRadius, 1.0, 0.6), n0);
...
void main() {
    // convert fragment coordinates to centered UV space (-1 to +1)
    vec2 uv = (gl_FragCoord.xy * 2.0 - u_resolution.xy) / u_resolution.y;

    vec4 col = drawCircle(uv);

    // black background
    vec3 bg = vec3(0.0);
    
    // mix bg with circle based on alpha
